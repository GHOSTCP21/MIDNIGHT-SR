/**********************************************
 *  MIDNIGHT S.R – ROULETTE AVEC PIN
 **********************************************/

// ⚙️ CODE PIN CONFIGURABLE
...
// On attend que le DOM soit prêt pour initialiser la roulette après validation du PIN
document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkPin() {
        if (pinInput.value.trim() === CORRECT_PIN) {
            ...
            initRoulette(); // Démarre la roulette une fois le PIN validé
        } else {
    ...
    // Initialisation de la roulette (appelée après PIN correct)
    function initRoulette() {
        // ========== RÉFÉRENCES DOM ROULETTE ==========
        ...
        function addParticipant(name) {
            ...
            if (!trimmed) return;
            if (trimmed.length > 30) { alert('Le nom ne doit pas dépasser 30 caractères.'); return; }
        ...
        function setModerator(name) {
            const trimmed = name.trim();
            if (!trimmed) return;
            if (trimmed.length > 40) { alert('Le nom du modérateur ne doit pas dépasser 40 caractères.'); return; }
        ...
        console.log('%c🌙 Midnight S.R %cRoulette prête.', 'font-size:1.4em;font-weight:bold;color:#d4a853;', 'color:#a8b8d4;');
    }
});